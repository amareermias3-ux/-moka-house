/* ================================================================
   MOKA HOUSE — v5.8 (Real manual-verify payments + full credit)
================================================================ */
...
/* ✍️ FULL CREDIT — name + email + BOTH phones (on every page) */
const CREDIT_HTML = `
<div style="display:flex;gap:1.2rem;flex-wrap:wrap;justify-content:center;align-items:center;padding:1.1rem 1rem 1.5rem;font-family:'Space Mono',monospace;font-size:.66rem;letter-spacing:.1em;color:#93866f;border-top:1px solid rgba(241,230,212,.12);margin-top:2.5rem;text-align:center">
  <span data-i18n="bot_credit">☕ ይህ ሮጀክት የተሰራው በ</span>&nbsp;<b style="color:#d98e32">ERMIAS AMARE</b>
...
(function(){
  ...
  var u=encodeURIComponent(location.href.split('?')[0]),t=encodeURIComponent('MOKA HOUSE ☕ Roastery & Espresso Bar');
...
function load() {
  try { fs.mkdirSync(path.dirname(DB_PATH), { recursive: true }); return JSON.parse(fs.readFileSync(DB_PATH, 'utf8')); }
    ...
    return { orders: [], newsletter: [], menu: SEED_MENU, events: SEED_EVENTS, posts: SEED_POSTS, knowledge: SEED_KNOWLEDGE, activity: [], sessions: [], users: [], transactions: [], wallet: { balance: 0, totalReceived: 0 } };
  }
}
const db = load();
function ensureSeedUsers() {
  ...
  db.menu = db.menu && db.menu.length ? db.menu : SEED_MENU;
  ...
  /* 💰 real payment destinations (owner's numbers) */
...
am:{register:'መለያ ለመፍር፦ Register ጽ ይ ም፣ ኢሜይል፣ ልክ እና የይለፍ ቃል ሞላ — ወይም "በGoogle ተመዝገ"',login:'በተጠቃ ስም ወይም ኢሜይል + የይለፍ ቃል ግባ — ወይም በGoogle።',wallet:'ቴሌ ብር፣ ኤም-ፔሳ PayPal፣ ካርድ፣ ባንክ እና ፕቶ እንቀበላለን።',stamps:'ስታምፕ ካርድ፦ በእያንዳንዱ መጠጥ አንድ ስታምፕ — 7 ገዝተህ 8ኛው ነፃ።',new:'አዲስ ዜና ',langs:'ይቱ በ8 ቋንቋዎች ይናገራል — ከላይ ኝ መቀየር ትችላለህ።',delivery:'እስካሁን አናደርስም — በሦስቱም ሱቆቻችን ተረ።'},
...
ti:{register:'ሕሳ ንምፍር ኣ Register ም፡ ኢመይል፡ ስልክን ፓስዎርድን መላእ — ወይ ብGoogle።',login:'ብስም ወይ ኢመይል + ፓስርድ እቶ ወይ Google',wallet:'Telebirr, M-Pesa, PayPal, ካርድ, ባንክን ክሪን ንበል',stamps:'ካርዲ፡ ነፍሲ ወከ መስተ ሓንቲ ስታም — 7 ግዛእ 8ይ ነ።',new:'ድ ዜና ',langs:'እቲ ሳይት 8 ቋንቋታት ይዛረ — ካብ ዕሊ ያይር',delivery:'ኣየጽሕን — ኣብ ሰለስተ ዱኳናትና ተቐበል።'},
...
function botReply(raw, lang) {
  ...
  if (/(pay|payment|telebirr|mpesa|paypal|card|bank|crypto|ክፍያ|ቴ|bixinta|pago|paiement|wallet)/.test(t)) return `${d.bot_pay}: Telebirr (${db.settings.payments.telebirr || '—'}) · M-Pesa (${db.settings.payments.mpesa || '—'}) · PayPal · Bank · Crypto.`;
...
const HTML_PAGES = { '/': 'index.html', '/index.html': 'index.html', '/login.html': 'login.html', '/register.html': 'register.html', '/admin.html': 'admin.html', '/track.html': 'track.html' };
app.get(Object.keys(HTML_PAGES), (req, res, next) => {
  const file = path.join(__dirname, 'public', HTML_PAGES[req.path]);
  fs.readFile(file, 'utf8', (err, html) => {
    if (err) return next();
    const injection = `<script>window.__I18N = ${JSON.stringify(I18N)};</script>${TRACK_FLOAT}${SHARE_FLOAT}${CREDIT_HTML}`;
...
/* ================= 💰 WALLET (real manual-verify payments) ================= */
app.post('/api/wallet/pay', payLimiter, (req, res) => {
  ...
  setTimeout(() => {
    ...
    res.status(201).json({ ok: true, order, tx, payTo: (db.settings.payments || {})[method] || null });
  }, 800);
});
...
app.get('/health', (req, res) => res.json({ ok: true, env: NODE_ENV, users: db.users.length, supabase: sbEnabled }));

if (!IS_VERCEL) {
  app.listen(PORT, () => console.log(`\n   ☕ MOKA HOUSE · ${NODE_ENV}\n   → http://localhost:${PORT}\n   💰 real payments: manual verify · ✍️ ERMIAS AMARE\n`));
}
export default app;