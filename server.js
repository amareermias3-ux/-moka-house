/* ================================================================
   MOKA HOUSE / ERSCOMAS — v7.6 (Table Reservations)
================================================================ */
...
const DB_PATH = IS_VERCEL ? '/tmp/db.json' : path.join(ROOT, 'data', 'db.json');
...
const SEED_MENU = [
 ...
 { id: 'spl', cat: 'Espresso', name: 'Spanish Latte', price: 5.00, desc: 'Espresso + sweet condensed milk.', seed: 'spanish-latte', tags: ['new'], available: true },
 { id: 'wmo', cat: 'Espresso', name: 'White Mocha', price: 5.00, desc: 'Espresso + white chocolate + cream.', seed: 'white-mocha', tags: ['new'], available: true },
 { id: 'mat', cat: 'Espresso', name: 'Matcha Latte', price: 5.20, desc: 'Ceremonial matcha + steamed milk.', seed: 'matcha-latte', tags: ['new'], available: true },
 { id: 'cha', cat: 'Espresso', name: 'Chai Latte', price: 4.80, desc: 'Spiced masala chai + milk.', seed: 'chai-latte', tags: ['new'], available: true },
 { id: 'cor', cat: 'Espresso', name: 'Cortado', price: 4.00, desc: 'Half espresso, half warm milk.', seed: 'cortado', tags: [], available: true },
 { id: 'emm', cat: 'Espresso', name: 'Espresso Macchiato', price: 3.40, desc: 'Espresso "stained" with foam.', seed: 'espresso-macchiato', tags: [], available: true },
 ...
 { id: 'mint', cat: 'Filter', name: 'Ethiopian Mint Tea', price: 3.00, desc: 'Fresh mint, soothing.', seed: 'mint-tea', tags: ['new'], available: true },
 { id: 'cham', cat: 'Filter', name: 'Chamomile Tea', price: 3.00, desc: 'Calming herbal infusion.', seed: 'chamomile-tea', tags: ['new'], available: true },
 { id: 'blk', cat: 'Filter', name: 'Black Tea', price: 2.80, desc: 'Strong breakfast tea.', seed: 'black-tea', tags: [], available: true },
 { id: 'hot', cat: 'Filter', name: 'Hot Chocolate', price: 4.20, desc: 'Belgian chocolate, steamed milk.', seed: 'hot-chocolate', tags: ['new'], available: true },
 ...
 { id: 'man', cat: 'Cold', name: 'Mango Smoothie', price: 5.50, desc: 'Ripe mango, yogurt, ice.', seed: 'mango-smoothie', tags: ['new'], available: true },
 { id: 'str', cat: 'Cold', name: 'Strawberry Smoothie', price: 5.50, desc: 'Fresh strawberries + milk.', seed: 'strawberry-smoothie', tags: ['new'], available: true },
 { id: 'avo', cat: 'Cold', name: 'Avocado Juice', price: 5.80, desc: 'The Addis favorite — creamy avocado.', seed: 'avocado-juice', tags: ['new'], available: true },
 { id: 'orj', cat: 'Cold', name: 'Fresh Orange Juice', price: 4.50, desc: 'Squeezed to order.', seed: 'orange-juice', tags: ['new'], available: true },
 { id: 'ict', cat: 'Cold', name: 'Iced Tea', price: 4.00, desc: 'Peach or lemon, over ice.', seed: 'iced-tea', tags: ['new'], available: true },
 { id: 'lad', cat: 'Cold', name: 'Mint Lemonade', price: 4.00, desc: 'Zesty, refreshing, minty.', seed: 'mint-lemonade', tags: ['new'], available: true },
 { id: 'milk', cat: 'Cold', name: 'Vanilla Milkshake', price: 5.50, desc: 'Thick, creamy, vanilla bean.', seed: 'vanilla-milkshake', tags: ['new'], available: true },
 ...
 { id: 'sco', cat: 'Bakes', name: 'Blueberry Scone', price: 3.50, desc: 'Crumbly, buttery, berry.', seed: 'blueberry-scone', tags: ['new'], available: true },
 { id: 'club', cat: 'Bakes', name: 'Club Sandwich', price: 6.50, desc: 'Triple-decker, chicken + egg.', seed: 'club-sandwich', tags: ['new'], available: true },
 { id: 'chk', cat: 'Bakes', name: 'Chicken Sandwich', price: 6.80, desc: 'Grilled chicken, garlic aioli.', seed: 'chicken-sandwich', tags: ['new'], available: true },
 { id: 'bur', cat: 'Bakes', name: 'Beef Burger', price: 7.00, desc: 'Smashed patty, cheddar, brioche.', seed: 'beef-burger', tags: ['new'], available: true },
 { id: 'piz', cat: 'Bakes', name: 'Pizza Slice', price: 5.50, desc: 'Wood-fired, mozzarella.', seed: 'pizza-slice', tags: ['new'], available: true },
 { id: 'pan', cat: 'Bakes', name: 'Pancake Stack', price: 5.50, desc: 'Fluffy, maple syrup, butter.', seed: 'pancakes', tags: ['new'], available: true },
 { id: 'waf', cat: 'Bakes', name: 'Belgian Waffle', price: 5.80, desc: 'Crisp, deep pockets, berries.', seed: 'waffles', tags: ['new'], available: true },
 { id: 'enk', cat: 'Bakes', name: 'Enkulal Firfir', price: 5.50, desc: 'Ethiopian scrambled eggs + spices.', seed: 'enkulal-firfir', tags: ['new'], available: true },
 { id: 'fat', cat: 'Bakes', name: 'Fatira', price: 4.50, desc: 'Crispy layered flatbread + honey.', seed: 'fatira', tags: ['new'], available: true },
];
const SEED_EVENTS = [
...
function ensureSeedUsers() {
  if (!db.users.some(u => u.user === ADMIN_USER)) db.users.push({ user: ADMIN_USER, hash: sha(ADMIN_PASS), role: 'admin', provider: 'password', permissions: null, points: 0, created: new Date().toISOString() });
  for (let i = 1; i <= MOD_COUNT; i++) { const name = 'moderator' + i; if (!db.users.some(u => u.user === name)) db.users.push({ user: name, hash: sha(MOD_PASS), role: 'moderator', provider: 'password', permissions: { ...DEFAULT_MOD_PERMS }, points: 0, created: new Date().toISOString() }); }
  db.users.forEach(u => { if (typeof u.points !== 'number') u.points = 0; });
...
am:{register:'መለያ ለመፍጠር፦ Register ጽ ላይ ስም፣ ኢሜይል፣ ልክ እና የይለፍ ል ሞላ — ወይም "በGoogle ተመዝገ"።',login:'በተጠሚ ስም ወይም ኢሜይል + የይለፍ ቃል ግባ — ወይም በGoogle።',wallet:'ቴሌብር፣ ኤም-ፔ፣ PayPal ካርድ ባን እና ክፕቶ እንቀበላለን።',stamps:'ስታምፕ ካርድ፦ በእያንዳንዱ መጠጥ አንድ ስታምፕ — 7 ግዛ፣ 8ኛው ነፃ።',new:'አዲስ ዜና፦ ',langs:'ሳይ በ8 ቋንቋዎች ናገራል — ከላይ ቀኝ መቀየር ትችላለህ።',delivery:'እስካሁን አናደርስም — በሦስቱም ሱቆችን ተከብ።'},
...
ti:{register:'ሕሳ ንምጣር፡ ኣብ Register ም፡ መይል ስልክን ፓስዎርድን መላእ — ወይ ብGoogle።',login:'ብስም ወይ ኢመይል + ፓስዎርድ እቶ፡ ወይ ብGoogle።',wallet:'Telebirr, M-Pesa, PayPal, ካርድ, ባንክን ክሪፕቶን ንቕበል።',stamps:'ካርዲ፡ ነፍሲ ወከፍ መስተ ሓንቲ ስታምፕ — 7 ግዛእ 8ይ ነጻ።',new:'ሓድሽ ዜና፦ ',langs:'እቲ ሳይት 8 ቋንቋታት ይዛረብ — ካብ ላዕሊ ቀያይር።',delivery:'ኣየብሕን — ኣብ ለስተ ኳናትና ቐበል'},
...
function botReply(raw, lang) {
  ...
  if (/(leaderboard|vip|top|loyal|ደረ)/.test(t)) return '🏆 VIP Leaderboard: /leaderboard.html — climb to Roaster 👑!';
  ...
  if (/(reserve|table|book|ጠረጴ|ቦታ ስያ)/.test(t)) return '🪑 Reserve a table: /reserve.html — book your seat!';
...
/* ===== injected: fx.js FIRST + floats + beacon ===== */
const CREDIT_HTML = `
...
const HTML_PAGES = { '/': 'index.html', '/index.html': 'index.html', '/login.html': 'login.html', '/register.html': 'register.html', '/admin.html': 'admin.html', '/track.html': 'track.html', '/quiz.html': 'quiz.html', '/story.html': 'story.html', '/friends.html': 'friends.html', '/terms.html': 'terms.html', '/wholesale.html': 'wholesale.html', '/gift.html': 'gift.html', '/subscribe.html': 'subscribe.html', '/receipt.html': 'receipt.html', '/leaderboard.html': 'leaderboard.html', '/wait.html': 'wait.html', '/reserve.html': 'reserve.html' };
app.get(Object.keys(HTML_PAGES), (req, res, next) => {
  const file = path.join(ROOT, 'public', HTML_PAGES[req.path]);
...
/* ================= 🏆 Leaderboard ================= */
function tier(p){ return p>=150?'Roaster 👑':p>=50?'Brewer ☕':p>0?'Bean 🌱':'New'; }
app.get('/api/leaderboard', (req, res) => {
  const top = db.users.filter(u=>(u.points||0)>0).sort((a,b)=>(b.points||0)-(a.points||0)).slice(0,10)
    .map((u,i)=>({ rank:i+1, name:u.name||u.user, points:u.points||0, tier:tier(u.points||0) }));
  res.json(top);
});

/* ================= 🪑 Table Reservations ================= */
app.post('/api/reserve', regLimiter, (req, res) => {
  const { name, phone, people, date, time, shop } = req.body || {};
  if (!name || !phone || !date || !time) return res.status(400).json({ error: 'Name, phone, date and time required' });
  db.reservations = db.reservations || [];
  db.reservations.unshift({ id: 'R' + Date.now(), name: String(name).slice(0, 60), phone: String(phone).slice(0, 20),
    people: Math.max(1, Math.min(12, parseInt(people) || 2)), date: String(date).slice(0, 10), time: String(time).slice(0, 5),
    shop: shop || 'Old Harbor', status: 'pending', created: new Date().toISOString() });
  log(`🪑 New reservation — ${name} (${people} ppl)`); save();
  res.status(201).json({ ok: true });
});
app.get('/api/reservations', requirePerm('can_orders'), (req, res) => res.json(db.reservations || []));
app.post('/api/reservations/:id/confirm', requireAdmin, (req, res) => {
  const r = (db.reservations || []).find(x => x.id === req.params.id); if (!r) return res.status(404).json({ error: 'Not found' });
  r.status = 'confirmed'; log(`✅ Confirmed reservation ${r.id}`); save(); res.json({ ok: true });
});
...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    if (!acct) {
      ...
      acct = { user: uname, hash: null, role: 'customer', provider: 'google', googleId: String(c.sub), email: c.email, name: c.name || uname, permissions: null, points: 0, created: new Date().toISOString() };
...
app.post('/api/admin/register', regLimiter, (req, res) => {
  ...
  const hash = sha(pass);
  db.users.push({ user: uname, hash, role: 'customer', provider: 'password', name: nm, email: em, phone: ph || null, gender: String(gender || '').slice(0, 10) || null, permissions: null, points: 0, created: new Date().toISOString() });
...
app.post('/api/admin/login', authLimiter, (req, res) => {
  ...
  if (!acct && typeof pass === 'string') {
    ...
    if (a && (a.user === uname || (a.email && a.email === uname)) && safeEq(sha(pass), a.hash)) {
      ...
      if (!db.users.some(u => u.user === a.user)) { db.users.push({ user: a.user, hash: a.hash, role: a.role || 'customer', provider: a.provider || 'password', email: a.email || null, name: a.name || null, permissions: null, points: 0, created: new Date().toISOString() }); save(); }
    }
  }
...
app.get('/api/admin/whoami', requireLogin, (req, res) => { const u = getUser(req); res.json({ user: u.user, role: u.role, permissions: u.permissions || null, email: u.email || null, points: u.points || 0 }); });

/* ================= USERS ================= */
app.get('/api/users', requireLogin, (req, res) => {
  const me = getUser(req);
  if (me.role !== 'admin') return res.json(db.users.map(u => ({ user: u.user, role: u.role })));
  res.json(db.users.map(u => ({ user: u.user, role: u.role, provider: u.provider || 'password', email: u.email || null, phone: u.phone || null, name: u.name || null, permissions: u.permissions || null, points: u.points || 0, created: u.created, sessions: 0 })));
});
...
app.post('/api/wallet/pay', payLimiter, (req, res) => {
  ...
  setTimeout(() => {
    ...
    if (me) { const u = db.users.find(x => x.user === me.user); if (u) { u.points = (u.points || 0) + Math.max(1, Math.round(subtotal)); } }
...
if (!IS_VERCEL) {
  app.listen(PORT, () => console.log(`\n   ☕ ERSCOMAS · ${NODE_ENV}\n   → http://localhost:${PORT}\n   🪑 Reservations · 🎮 Wait games · 🛡️ Security · ✍️ ERMIAS AMARE\n`));
}
export default app;